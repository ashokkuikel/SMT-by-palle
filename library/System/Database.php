...
/**
 * PDO gesteuerte Datenbanklasse
 *
 * <code>
 * $db = Databse::getInstance();;
 * </code>
 *
...
class Database {
   ...
   * @param bool $return        	
   */
  public function getQuery($query, $value = '', $return = FALSE) {
    try {
      
      if (class_exists('Blaetter')) {
        $ba = Blaetter::getInstance();
        $real_query = $query;

        if (is_int($ba->start) && (preg_match("/select/i", $query))) {
          $query .= " LIMIT " . $ba->get('start') . ", " . $ba->get('anzahl');
        }
      }
